#include "SMPhysicalPrinterDialog.hpp"
...
SMPhysicalPrinterDialog::SMPhysicalPrinterDialog(wxWindow* parent) :
    DPIDialog(parent, wxID_ANY, _L("Physical Printer"), wxDefaultPosition, wxSize(45 * wxGetApp().em_unit(), -1), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    btnOK->Bind(wxEVT_BUTTON, &SMPhysicalPrinterDialog::OnOK, this);
...
SMPhysicalPrinterDialog::~SMPhysicalPrinterDialog()
{
}

void SMPhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup* m_optgroup)
{
...
void SMPhysicalPrinterDialog::update_ports() {
...
void SMPhysicalPrinterDialog::update_webui()
{
    const PrinterTechnology tech = Preset::printer_technology(*m_config);
    if (tech == ptFFF) {
        const auto opt = m_config->option<ConfigOptionEnum<PrintHostType>>("host_type");
...
void SMPhysicalPrinterDialog::update_printhost_buttons()
{
    std::unique_ptr<PrintHost> host(PrintHost::get_print_host(m_config));
    if (host) {
...
void SMPhysicalPrinterDialog::update_preset_input() {
...
void SMPhysicalPrinterDialog::update(bool printer_change)
{
...
void SMPhysicalPrinterDialog::update_host_type(bool printer_change)
{
    if (m_config == nullptr)
        return;
...
void SMPhysicalPrinterDialog::update_printers()
{
    wxBusyCursor wait;
...
void SMPhysicalPrinterDialog::on_dpi_changed(const wxRect& suggested_rect)
{
...
void SMPhysicalPrinterDialog::OnOK(wxEvent& event)
{
